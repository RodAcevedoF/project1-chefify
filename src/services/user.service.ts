...
export const UserService = {
	...
	async getUserByEmail(email: string): Promise<Omit<IUser, 'password'>> {
		...
		async deleteUser(id: string): Promise<Omit<IUser, 'password'> | null> {
			...
			// return null to indicate deletion, or return sanitized deleted user if repo returns it
			return null;
		},

		async saveRecipe(userId: string, recipeId: string): Promise<Omit<IUser, 'password'>> {
		...
		async deleteRecipe(userId: string, recipeId: string): Promise<Omit<IUser, 'password'> | null> {
			const user = await UserRepository.findById(userId);
			if (!user) throw new NotFoundError('User not found');
